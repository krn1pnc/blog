...
1. 若 $[1, i - 1]$ 中有被 ban 的值，且 $i$ 位置要求填 $\le i - 1$ 的数字（不做要求视作要求填 $0$），那么若 $p_{i - 1} \le i$，那么我们无法选出满足条件的值，故舍去这种情况．剩下情况总能找到唯一的一个值选取，有转移 $f_{i, 0} \leftarrow f_{i - 1, 2}$．

2. 若 $[1, i]$ 中有被 ban 的值，且 $i$ 位置要求填 $\le i$ 的数字，那么有转移 $f_{i, 0} \leftarrow f_{i - 1, 0} + f_{i - 1, 1}$．
